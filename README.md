#:musical_note: Vibes Radio PC

## :computer: Aperçu

Vibes Radio PC est une expérience d'écoute pour els radio international qui sont optimisée pour le grand écran avec :
- **38 radios internationales** de 8 pays
- **Interface immersive en plein écran** avec animations saisonnières
- **4 thèmes saisonniers** : Hiver :snowflake:, Printemps :cherry_blossom:, Été :sunny:, Automne :fallen_leaf:
- **Easter Eggs interactifs** cachés dans l'interface
- **Design** moderne et élégant

## :sparkles: Fonctionnalités

### :radio: Radios Internationales
- **38 stations** gratuit est sans problème (sauf la radio japon qui est down)
- **8 pays** : France, US, Allemagne, Espagne, Italie, Japon (down), Belgique, Maroc
- Streaming de haute qualité en direct 24/7
- Contrôles intuitifs comme toute radios (play/pause)
- Interface épurée et moderne

### :rainbow: Thèmes Saisonniers
- **Changement automatique** saison synchronisé avec les date internationale
- **Changement manuel** dans le Centre de Contrôle
- **Animations immersives** pour chaque saison :
  - :snowflake: **Hiver** : de la neige est tombants
  - :cherry_blossom: **Printemps** : des fleurs flottantes avec des papillons
  - :sunny: **Été** : rayon de soleil avec des émoticône tombants
  - :fallen_leaf: **Automne** : fête d'halloween dons des feuille qui tonbant

### :video_game: Easter Eggs
**Des Easter Eggs cachés**: à découvrir ?

### :art: Design
- **Interface glassmorphism** avec effets de blur
- **Animations fluides** et performantes
- **Responsive** : S'adapte à toutes les tailles d'écran
- **Mode plein écran** recommandé pour l'expérience optimale
- **Particules saisonnières** en arrière-plan

## :rocket: Technologies

- **HTML5** - Structure
- **CSS3** - Animations et glassmorphism
- **JavaScript Vanilla** - Logique et interactions
- **HTML5 Audio API** - Streaming radio
- **Canvas API** - Animations de particules

### :globe_with_meridians: En ligne
Accède directement à l'application :
```
https://tfasty.github.io/winter-vibes-radio/
```

## :dart: Utilisation

### Navigation
- **Sélection radio** : Clique sur une station dans la liste
- **Play/Pause** : Bouton central ou Espace
- **Changer de saison** : Triple-clic sur les emojis saisonniers (:snowflake::cherry_blossom::sunny::fallen_leaf:)
- **Mode plein écran** : F11 (recommandé)

### Raccourcis clavier
- **Espace** : Play/Pause
- **F11** : Mode plein écran
- **Esc** : Quitter le plein écran

## :earth_africa: Radios Disponibles

### :flag_fr: France (9 radios)
FIP, Noël Radio, TSF Jazz, France Musique, FIP Electro, FIP Rock, FIP Groove, J-Pop Powerplay, Skyrock

### :flag_us: English (4 radios)
Chrismas UK, Jazz FM, Heart Radio, Smooth FM

### :flag_de: Allemagne (9 radios)
I Love Chill, I Love Christmas, I Love Jazz, I Love Relax, I Love Dance, I Love Rock, I Love Radio, I Love Lounge, Anime DE

### :flag_es: Espagne (4 radios)
Novidad, Jazz ES, Los 40, Anime ES

### :flag_it: Italie (4 radios)
Radio 105, Virgin Radio, Radio Monte Carlo, Natale

### :flag_jp: Japon (1 radios)
Listen.moe (down)

### :flag_ma: Maroc (2 radios)
Noël MA, Jazz MA

## :art: Thèmes Saisonniers

### :snowflake: Hiver (Décembre - Février)
- **Couleurs** : Bleu glacé, blanc, violet
- **Particules** : Flocons de neige tombants
- **Ambiance** : Froid et cosy

### :cherry_blossom: Printemps (Mars - Mai)
- **Couleurs** : Rose, vert tendre, jaune pastel
- **Particules** : Fleurs flottantes + papillons
- **Ambiance** : Fraîche et fleurie

### :sunny: Été (Juin - Août)
- **Couleurs** : Jaune, orange, bleu ciel
- **Particules** : Soleil pulsant avec rayons
- **Ambiance** : Chaude et lumineuse

### :fallen_leaf: Automne (Septembre - Novembre)
- **Couleurs** : Orange, marron, rouge
- **Particules** : Feuilles tourbillonnantes
- **Ambiance** : Douce et chaleureuse

## :handshake: Contribution

Les contributions sont les bienvenues ! N'hésite pas à :
- Signaler des bugs
- Proposer des nouvelles fonctionnalités
- Ou en tout envoient un mails sur notre contact pro

## :tada: Remerciements

Merci à tous les streamers et radios pour leurs flux audio !

## :bulb: Astuces

- :desktop: **Utilise le mode plein écran (F11)**
- :art: **Explore les Easter Eggs** 
- :rainbow: **Change de saison**
- :loud_sound: **Règle le volume**
- :sparkles: **Laisse tourner**

---

:star: **Si tu aimes le projet, n'oublie pas de mettre un avis !** :star:

:musical_note: **Bonne écoute sur Vibes Radio **:musical_note:
